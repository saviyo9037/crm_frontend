...
import { toggleleadsourceeditModal } from '../../redux/settingsmodalSlice'
import { editleadsourcesettings } from '../../services/settingservices/leadSourceSettingsRouter'
import { useFormik } from 'formik'
import * as Yup from 'yup'
import Spinner from '../Spinner'

function Leadsourceeditmodal() {

    const dispatch = useDispatch()
    const queryclient = useQueryClient()
    const selectedsource = useSelector((state) => state.settingsmodal.selectedSource)

    const [showsuccess, setshowsuccess] = useState(false)

    const updatingleadsource = useMutation({
        mutationKey: ['Editing Lead Source'],
        mutationFn: editleadsourcesettings,
        onSuccess: () => {
    ...
    const editleadsourceForm = useFormik({
        initialValues: {
        ...
        onSubmit: async (values) => {
            await updatingleadsource.mutateAsync({ settingsleadsourceId: selectedsource._id, settingsleadsourcedata: values })
            setshowsuccess(true)
            setTimeout(() => {
                dispatch(toggleleadsourceeditModal(null))
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50 px-2 sm:px-4">
            ...
            >
                <button
                    onClick={() => dispatch(toggleleadsourceeditModal(null))}
                    className="absolute top-2 sm:top-3 right-3 sm:right-4 text-xl sm:text-2xl text-gray-400 hover:text-red-500 transition"
                >
                    &times;
                </button>

                {updatingleadsource.isPending && (
                    <Spinner />
                )}
                {updatingleadsource.isError && (
                    <p className='text-red-600 bg-red-100 p-2 sm:p-3 rounded-md mb-3 sm:mb-4 text-sm sm:text-base'>
                        {updatingleadsource.error?.response?.data?.message}
                    </p>
                )}

                <form onSubmit={editleadsourceForm.handleSubmit} className="space-y-3 sm:space-y-4">
                    <div className="flex flex-col">
                        ...
                        <input
                            type="text"
                            name="title"
                            value={editleadsourceForm.values.title}
                            {...editleadsourceForm.getFieldProps('title')}
                        ...
                        />
                        {editleadsourceForm.errors.title && editleadsourceForm.touched.title && (
                            <span className="text-red-500 text-xs sm:text-sm mt-1">{editleadsourceForm.errors.title}</span>
                        )}
                    </div>
            ...
            <AnimatePresence>
                {showsuccess && (
                    ...
                    >
                        ...
                        >
                            ✅ Lead source edited successfully!
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>
