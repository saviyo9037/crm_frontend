...
import { updatesettingsgst } from '../../services/settingservices/gstSettingsRouter'

function Gsteditmodal() {

    const dispatch = useDispatch()
    const queryclient = useQueryClient()
    const selectedgst = useSelector((state) => state.settingsmodal.selectedGstModal)

    const [showsuccess, setshowsuccess] = useState(false)

    const updatinggst = useMutation({
        mutationKey: ['Updating Documents'],
        mutationFn: updatesettingsgst,
        onSuccess: () => {
            queryclient.invalidateQueries(['List Gst'])
        }
    })

    const gstvalidation = Yup.object({
        gst_amount: Yup.number()
    })

    const editgstform = useFormik({
        initialValues: {
            gst_amount: selectedgst?.gst_amount || ''
        },
        validationSchema: gstvalidation,

        onSubmit: async (values) => {
            await updatinggst.mutateAsync({
                gstDataId: String(selectedgst?._id),
                gst_amount: Number(values.gst_amount)
            })
            setshowsuccess(true)
            setTimeout(() => {
                dispatch(togglegsteditModal(null))
                ...
                <form onSubmit={editgstform.handleSubmit} className="space-y-3 sm:space-y-4">
                    <div className="flex flex-col">
                        ...
                        <input
                            type="text"
                            name="gst_amount"
                            value={editgstform.values.gst_amount}
                            {...editgstform.getFieldProps('gst_amount')}
                        ...
                        />
                        {editgstform.errors.gst_amount && editgstform.touched.gst_amount && (
                            <span className="text-red-500 text-xs sm:text-sm mt-1">{editgstform.errors.gst_amount}</span>
                        )}
                    </div>
                    <button
                        type="submit"
